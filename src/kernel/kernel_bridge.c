...
static void bridge_KeSetEvent(void)
{
    ...
    /* Two different things arrive here. A title that called KeInitializeEvent
     * built a DISPATCHER_HEADER in its own memory and polls SignalState
     * directly, so that object has to be set in place -- a host SetEvent on a
     * shadow handle would leave the word the guest is spinning on untouched.
     * bridge_guest_event returns non-NULL only for exactly that shape, so when
     * it declines, the VA names a handle and the shadow table resolves it. */
    if (state) {
...
static void kernel_thunk_dispatch(void)
{
    ...
    if (ordinal == g_kcaller_ordinal)
        kcaller_record(g_esp ? BRIDGE_MEM32(g_esp) : 0);
